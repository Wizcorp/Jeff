...
Jeff.prototype._init = function (params, cb) {
	...
	// Primary options
	this._inputDir          = params.inputDir          || '.'; // Source of the file(s) to export. Can be defined as a regular expression 
	this._outDir            = params.outDir            || '.';
	this._source            = params.source            || '*.swf';

	// Secondary options
	this._scope             = params.scope             || 'main';
	this._renderFrames      = params.renderFrames      || false;
	this._ratio             = params.ratio             || 1;

	// Optimisation options
	this._imageOptim        = params.imageOptim        || false;
	...
	this._customReadFile    = params.customReadFile;
	this._fixedSize         = params.fixedSize;

	// Whether only one frame is being rendered
	// Boolean used for naming purpose
	this._onlyOneFrame      = (this._renderFrames instanceof Array) && (this._renderFrames.length === 1);

	// Checking for uncompatible options
	if (this._renderFrames && this._simplify) {
		this._simplify = false;
		console.warn('[Jeff] Option to simplify will be ignored');
	}

	if (this._renderFrames && this._flatten) {
		this._flatten = false;
		console.warn('[Jeff] Option to flatten will be ignored');
	}

	if (this._renderFrames && this._compressMatrices) {
		this._compressMatrices = false;
	...
	if (this._source instanceof Array) {
		...
		glob(this._source, { cwd: this._inputDir }, function (error, uris) {
			cb(uris);
		})
	}
};
...
Jeff.prototype._parseFile = function (swfName, nextSwfCb) {
	...
	function onFileRead(error, swfData) {
		...
		self._parser.parse(swfName, swfData,
			function (swfObject) {
				var id = swfObject.id;
				// console.log('object', beautify(JSON.stringify(swfObject), { indent_size: 4 }));
				if (id === undefined) {
	...
	if (this._customReadFile) {
		this._customReadFile(swfName, onFileRead);
	} else {
		fs.readFile(path.join(this._inputDir, swfName), onFileRead);
	}
};
...
Jeff.prototype._generateImageName = function (imgName) {
	...
	if (!this._createAtlas && (!this._onlyOneFrame || Object.keys(this._classGroupList).length > 1)) {
