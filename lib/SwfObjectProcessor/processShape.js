...
function processShape(edges) {
	...
	for (var e = 0; e < edges.length; e += 1) {
		...
		if (edgeSet.line) {
			lineShapes.push({
				...
				lineStyle: JSON.parse(JSON.stringify(edgeSet.line, convertWidthToPixels)),
		...
		if (edgeSet.leftFillIdx) {
			// shapes with left fills are reversed
			var reversedRecords = [];
			for (var r = records.length - 1; r >= 0; r -= 1) {
			...
			var leftFillShape = {
				...
				fillStyle: JSON.parse(JSON.stringify(edgeSet.leftFill)),
				styleIdx: edgeSet.styleIdx,
				fillIdx: edgeSet.leftFillIdx
			};

			fillShapes.push(leftFillShape);
		}

		if (edgeSet.rightFillIdx) {
			var rightFillShape = {
				records: JSON.parse(JSON.stringify(records)),
				fillStyle: JSON.parse(JSON.stringify(edgeSet.rightFill)),
				styleIdx: edgeSet.styleIdx,
				fillIdx: edgeSet.rightFillIdx
			};

			fillShapes.push(rightFillShape);
		}
	}
	...
	for (idx = 0; idx <= maxStyleIdx; idx += 1) {
		...
		// TODO: document the reconstruction
		for (i = 1; i < idxFills.length; i += 1) {
