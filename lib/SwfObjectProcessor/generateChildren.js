			...
			if (objectData) {
				...
				for (var p in objectData) {
					if (   p !== 'clipDepth'
						...
						&& p !== 'name'        // not supported, but what is it useful for?
					) {
	...
	for (var i = 0; i < orderedDepths.length; i += 1) {
		...
		for (var idString in content) {
			...
			for (var j = 0; j < nFrames; j += 1) {
				...
				if (frameData.blendMode) blendings.push(frameData.blendMode);
				...
				if (blendings.length > 0) matrixSeq.blendings = blendings;
