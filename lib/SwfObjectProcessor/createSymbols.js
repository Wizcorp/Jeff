...
function createSymbol(swfObject) {
	...
	case 'shape':
		...
		if (imagesArray.length > 0) {
			...
			// TODO: make the following code work by updating SwfCanvasRenderer
			// // Attaching image list to shapes for direct access when rendering
			// for (var idx = 0; idx < shapes.length; idx += 1) {
			// 	var idxFills = shapes[idx].fills;
			// 	for (var f = 1; f < idxFills.length; f += 1) {
			// 		idxFills[f].images = imagesObject;
			// 	}
			// }
		}
...
function createSymbols(swfObjects) {
	...
	for (var s = 0; s < nbSymbols; s += 1) {
		symbols[s] = createSymbol(swfObjects[s]);
	}
	return symbols;
}
