...
SwfImgRenderer.prototype._renderPng = function (swfObject, whenDone) {
	...
	var self = this;
// console.error('_renderPng', swfObject)
...
SwfImgRenderer.prototype._translateJpg = function (swfObject, whenDone) {
	...
	if (!swfObject.alphaData) {
		swfObject.jpgContent = new Buffer(swfObject.data);
...
// console.error('swfObject', swfObject)
	// imgData contain the image color obtained from the palette, not adding the palette to the pngWriter
	var pngWriter = new PngWriter(swfObject.width, swfObject.height, true, 0, false);
...
// var buf = new Buffer(swfObject.colorTableSize * 3, 'binary')
// swfObject.data.copy(buf, 0, 0, swfObject.colorTableSize * 3);
// console.error('buf', JSON.stringify(buf))

// console.error('imgData.length', imgData.length, swfObject.width, swfObject.height, 4 * swfObject.width * swfObject.height)
...
SwfImgRenderer.prototype._translateSwfPngToPng = function (swfObject) {
	...
	var pxData = new Buffer(width * height * 4 + height); //+height for filter byte (1 per line)
	...
	for (var j = 0; j < height; j += 1) {
		...
		for (var i = 0; i < width; i += 1) {
			...
			if (colorTableSize) {
				...
				// is the using a color table the only reason for alpha being the last component of the pixel color?
				if (withAlpha) {
					alpha = data[idx + 3];
