...
function BoxPartitioning(upperBounds, priorityZone) {
	this.freeSpace = new DoublyList();
	this.freeSpace.add(new Box(undefined, upperBounds.left, upperBounds.right, upperBounds.top, upperBounds.bottom));
	...
	this.priorityZone = new Box(undefined, priorityLeft, priorityRight, priorityTop, priorityBottom);

	this.nRejections = 0;
}
module.exports = BoxPartitioning;

BoxPartitioning.prototype.fragmentFreeSpace = function (newBox) {
	...
	var freeSpacePtr = this.freeSpace.getFirst();
	...
	for (var i = 0; i < newFreeSpaceBoxes.length; i += 1) {
		...
		freeSpacePtr = this.freeSpace.getFirst();
		while (freeSpacePtr !== null) {
			freeBox = freeSpacePtr.object;
			freeSpacePtr = freeSpacePtr.next;
		...
		if (insert) {
			...
			this.freeSpace.add(newFreeSpaceBoxes[i]);
		}
	}
};
...
BoxPartitioning.prototype.add = function (element, w, h) {
	...
	while (freeSpacePtr !== null) {
		...
		if (w <= freeBox.w && h <= freeBox.h) {
			...
			var areaW = (x + w > this.priorityZone.r) ? x + w : this.priorityZone.r;
			var areaH = (y + h > this.priorityZone.b) ? y + h : this.priorityZone.b;
	...
	this.fragmentFreeSpace(elementBox);

	this.priorityZone.reset(
		(elementBox.l < this.priorityZone.l) ? elementBox.l : this.priorityZone.l,
		(elementBox.r > this.priorityZone.r) ? elementBox.r : this.priorityZone.r,
		(elementBox.t < this.priorityZone.t) ? elementBox.t : this.priorityZone.t,
		(elementBox.b > this.priorityZone.b) ? elementBox.b : this.priorityZone.b
	);
