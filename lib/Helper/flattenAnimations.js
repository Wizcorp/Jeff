...
function flattenInstance(symbols, instance) {
	...
	var c, children;
	if (symbol.isGraphic || symbol.className || instance.maskStart || instance.maskEnd || instance.filters || instance.blendings) {
		...
		if (symbol.isAnim) {
			...
			for (c = children.length - 1; c >= 0; c -= 1) {
				flattenInstance(symbols, children[c]);
			}
		}
	...
	children = symbol.children;
	for (c = children.length - 1; c >= 0; c -= 1) {
		Array.prototype.unshift.apply(tmpChildren, flattenInstance(symbols, children[c]));
	}
...
function flattenAnimations(symbols) {
	...
	for (id in classes) {
		...
		for (var c = children.length - 1; c >= 0; c -= 1) {
			Array.prototype.unshift.apply(newChildren, flattenInstance(symbols, children[c]));
		}
