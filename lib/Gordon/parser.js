...
SwfParser.prototype = {
	...
	parse: function (swfName, swfData, ondata, cb) {
		...
		this.ondata = ondata;
		var s = this._stream = new Stream(swfData);
		this._readSwlTable(s); //symbol table not used
		var sign = s.readString(3),
			v = Base.validSignatures;
		this.version = s.readUI8();
		this.fileLen = s.readUI32();
	...
	_parseMainFrame: function (s, cb) {
		...
		do {
			...
			while (true) {
				...
				numTags++;
				if (this[handl]) { this._handleTag(handl, s, len, frm); }
				else { this._handleUnhandledTag(handl, code, s, s.offset, len); }
				if (this.aborting) {
	...
	_handleDefineSprite: function (s, offset, taglen) {
		...
		do {
			...
			do {
				...
				if (sprTags.indexOf(code) >= 0 && this[handl]) { this._handleTag(handl, s, len, frm); } //this[handl](s,s.offset,len,frm)
				else { this._handleUnhandledTag(handl, code, s, s.offset, len, frm); }
	...
	_handleDefineScalingGrid: function (s, offset, len) {
		var grid = {
			type: 'scalingGrid',
			appliedTo: s.readUI16(),
			rect: s.readRect(),
		};
		this.ondata(grid);
	},
	_handleDefineSceneAndFrameLabelData: function (s, offset, len) {
		this._skipEndOfTag('DefineSceneAndFrameLabelData', true, s, offset, len);
	},
	...
	_handleDefineShape: function (s, offset, len, frm, withAlpha, v4shape) {
		...
		var fillStyles = this._readFillStyles(s, withAlpha);
		var lineStyles = this._readLineStyles(s, withAlpha, /*morph=*/ false, /*v2lineStyle=*/ v4shape);
		shape.edges    = this._readEdges(s, fillStyles, lineStyles, withAlpha, /*morph=*/ false, /*v2edges=*/ v4shape);
	...
	_handlePlaceObject3: function (s, offset, len, frm, v2place) {
		...
		var flags        = s.readUI8();
		var flags2       = v2place ? 0 : s.readUI8();
		var depth        = s.readUI16();
		var hasCharacter = flags  & f.HAS_CHARACTER;
		var hasImage     = flags2 & f.HAS_IMAGE;
		var className    = ((flags2 & f.HAS_CLASS_NAME) || (hasImage && hasCharacter)) ? s.readString : undefined;
	...
	_handleDefineBits: function (s, offset, len, frm, withAlpha) {
		...
		var data;
		var jpegTables = this._jpegTables;
		if (withAlpha) {
		...
		img.data = jpegTables ? jpegTables.substr(0, jpegTables.length - 2) + data.substr(2) : data;
		this.ondata(img);
		this._dictionary[id] = img;
	},
	...
	_handleDefineButton: function (s, offset, len, frm, advanced) {
		...
		this._skipEndOfTag('Button-action', true, s, offset, len);
		this.ondata(button);
		d[id] = button;
	},
	...
	_handleDefineFont: function (s) {
		...
		while (numGlyphs--) { glyphs.push(this._readEdges(s)); }
		this.ondata(font);
	...
	_handleDefineFont3: function (s, offset, len) {
		...
		s.readUI8(); // font name length
		font.fontName = s.readString();

		var n;
		var numGlyphs   = s.readUI16();
		var offsetTable = [];

		n = numGlyphs;
		while (n--) { font.useWideOffsets? s.readUI32(): s.readUI16(); } // offsetTable
		font.useWideOffsets? s.readUI32(): s.readUI16(); // codeTableOffset

		var glyphs = [];
		n = numGlyphs;
		while (n--) { glyphs.push(this._readEdges(s)); }
		font.glyphs = glyphs;

		var codeTable = [];
		n = numGlyphs;
		while (n--) { codeTable.push(s.readUI16()); }
		font.codeTable = codeTable;

		if (font.hasLayout) {
			// N.B:  Not tested
			// TODO: This part of the code needs to be checked
			font.fontAscent  = s.readUI16();
			font.fontDescent = s.readUI16();
			font.fontLeading = s.readSI16();
			...
			n = numGlyphs;
			while (n--) { advanceTable.push(s.readUI16()); }
			font.advanceTable = advanceTable;

			n = numGlyphs;
			while (n--) { boundsTable.push(s.readRect()); }
			...
			while (kerningCount--) {
				kerningTable.push({
					code1: isUTF8 ? s.readUI16() : s.readUI8(),
					code2: isUTF8 ? s.readUI16() : s.readUI8(),
					adjustment: s.readUI16()
				});
			}
