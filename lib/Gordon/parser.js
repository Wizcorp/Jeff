...
SwfParser.prototype = {
	...
	_handleDefineFont2: function (stream, offset, len) {
		...
		if (hasLayout) {
			f.ascent  = stream.readUI16();
			f.descent = stream.readUI16();
			f.leading = stream.readUI16();
			var advanceTable = f.advanceTable = [];
			var boundsTable  = f.boundsTable = [];
			var kerningTable = f.kerningTable = [];
			i = numGlyphs;
	...
	_handleDefineFont3: function (stream, offset, len) {
		...
		stream.readUI8(); // font name length
